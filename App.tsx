...
const App: React.FC = () => {
  const [currentMission, setCurrentMission] = useState<MissionType | null>(
    null
  );
  ...
  const handleMissionSelect = (mission: MissionType) => {
    ...
    setTimeout(() => {
      document
        .getElementById("nexus-form-container")
  ...
  const handleFormSubmit = async (data: NexusFormData) => {
    ...
    try {
      // Try secure submission first
      if (serverAvailable) {
        const response = await submitToNexusSecure(currentMission, data);
        ...
        // Fallback to local submission with Gemini
  ...
  return (
    <div className="min-h-screen bg-nexus-dark bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-900 via-nexus-dark to-black text-slate-200 selection:bg-nexus-accent selection:text-white font-sans overflow-x-hidden">
      ...
      <header className="relative z-10 p-6 md:p-10 text-center">
        ...
        <div className="absolute top-4 right-4 flex items-center gap-2 px-3 py-1 rounded-full bg-slate-800/50 border border-slate-700 text-xs">
          <div
            className={`w-2 h-2 rounded-full ${
              serverAvailable ? "bg-nexus-success" : "bg-nexus-danger"
            }`}
          ></div>
          <span className="text-slate-300">
            {serverAvailable ? "🔒 Sécurisé" : "⚠️ Mode local"}
          </span>
        </div>
          ...
          <div className="flex flex-col items-center space-y-12">
            ...
            <section className="text-center max-w-3xl mx-auto">
              <div className="inline-flex items-center px-4 py-2 rounded-full bg-slate-800/50 border border-slate-700 text-sm mb-6">
                ...
                <span className="font-mono text-nexus-success">
                  Système en ligne... En attente d'input utilisateur.
                </span>
              </div>
              ...
              <p className="text-slate-400">
                L'association a besoin de vous. Sélectionnez une mission
                ci-dessous pour ouvrir le canal de communication approprié.
              </p>
            </section>
