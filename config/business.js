// Configuration des données métier (JS runtime copy of business.ts)
// Exported as ESM so server.js can import './config/business.js'
    ...
    projects: [
      {
        ...
        progress: 65,
        description:
          "Migration vers une infrastructure scalable et décentralisée",
        funding: 15000,
        funded: 9750,
      },
      {
...
export const getDonationImpact = (amount, year = 2025) => {
  ...
  if (amount < 10) {
    ...
    return {
      ...
      emoji: "⚔️",
      impact: `${Math.floor(
        (amount / goals.projects[1].funding) * 100
      )}% du financement pour la Plateforme Collaborative`,
    ...
    return {
      ...
      emoji: "👑",
      impact: `${Math.floor(
        (amount / goals.totalFunding) * 100
      )}% de l'objectif annuel ${year}!`,
...
export const getAnnualObjectiveMessage = (year = 2025) => {
  const goals = ANNUAL_GOALS[year] || ANNUAL_GOALS[2025];
  const percentage = Math.round((goals.totalFunded / goals.totalFunding) * 100);
...
export const getGratitudeMessage = (mission, data, year = 2025) => {
  const username = (data && data.username) || "Voyageur";
  ...
  const messages = {
    ...
    DONATION: `Un immense "GG", ${username}! 🏆 Ton "Don de Ressources" 💎 de ${
      data && data.amount
    }€ est une bénédiction pour notre cause 🙏. 
Ce soutien ${
      data && data.isAnonymous ? "anonyme" : "nommé"
    } permettra de financer nos projets technologiques 🚀 et d'étendre notre communauté en ${year}. 
Ton impact est immense! 🌟 Merci de croire en notre mission! ❤️`,

    VOLUNTEER: `Bienvenue, ${username}! 🛡️ Tu as rejoint la "Guilde des Bénévoles" ! 🎖️ Tes compétences en ${
      data && data.skills
    } seront précieuses pour nos initiatives en ${year}.
Les "Gardiens du Nexus" 🗡️ t'accueillent chaleureusement. Reste connecté pour connaître tes premières missions! 📬`,

    INFO: `Accès Accordé, ${username}! 🔓 Ta requête sur le ${
      data && data.topic === "general" ? "domaine général" : data && data.topic
    } a été enregistrée dans nos archives 📚 en ${year}. 
Nos "Sages du Nexus" 🧙 analyseront ta question et te fourniront une réponse détaillée sous peu. 🌍`,
  };

  return (
    messages[mission] ||
    `Mission acceptée, ${username}. Merci de ton soutien en ${year}! ✨`
  );
};
