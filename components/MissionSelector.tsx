...
import { MessageSquare, Heart, Shield, HelpCircle, LucideIcon } from 'lucide-react';
...
const MissionCard: React.FC<MissionCardProps> = ({ 
  type, icon: Icon, title, description, color, onClick, isSelected 
}) => {
  const baseClasses = "relative p-6 rounded-xl border-2 transition-all duration-300 cursor-pointer overflow-hidden group hover:scale-105";
  ...
  const style = isSelected ? { borderColor: color, boxShadow: `0 0 15px ${color}40` } : {};
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-4">
        <Icon className="w-8 h-8 transition-transform group-hover:rotate-12" style={{ color: isSelected ? color : '#94a3b8' }} />
        {isSelected && <div className="h-2 w-2 rounded-full animate-ping" style={{ backgroundColor: color }} />}
      </div>
      ...
      <div className="absolute -bottom-4 -right-4 w-24 h-24 rounded-full opacity-5 pointer-events-none" style={{ backgroundColor: color }} />
    </div>
  );
};
...
export const MissionSelector: React.FC<MissionSelectorProps> = ({ selectedMission, onSelect }) => {
  return (
